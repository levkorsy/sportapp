...
class BasketballList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      Basketballgames: [],
  ...
  componentDidMount() {
    axios
      ...
      .then(
        function(response) {
          this.setState({ Basketballgames: response.data });
        }.bind(this)
      )
  ...
  render() {
    let allBasketballgames = this.state.Basketballgames.map(game => (
